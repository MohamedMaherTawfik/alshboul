<nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav">
        ...
        <li class="nav-item d-none d-sm-inline-block">
            <a href="{{ route('user.dashboard') }}" class="nav-link">الصفحة الرئيسية</a>
        </li>
        <li class="nav-item d-none d-sm-inline-block">
    ...
    <ul class="ml-auto navbar-nav">
        ...
        @php
            ...
            if (Auth::check()) {
                ...
                $notifications = collect(); // قائمة فارغة في حالة عدم وجود مستخدم
            }
        @endphp
        <li class="nav-item dropdown">
            <a class="nav-link" data-toggle="dropdown" href="#">
            ...
            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                ...
                @foreach ($notifications as $item)
                    <a href="{{ route('notifications.read', $item->id) }}" class="dropdown-item">
                        <p class="text-sm">{{ $item->data['title'] }}</p>
                        <p class="text-sm text-muted"><i
                                class="mr-1 far fa-clock"></i>{{ $item->created_at->diffForHumans() }}</p>
                    </a>
                @endforeach
                {{-- <div class="dropdown-divider"></div>
                <a href="#" class="dropdown-item">
