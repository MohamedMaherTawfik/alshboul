...
@section('main_title_content', 'إضافة وكالة جديدة')
@section('title_content', 'إنشاء')
@section('link_content')
    <a href="{{ route('agencies.index', $main) }}">قائمة الوكالات</a>
@endsection

@section('content')
    <div class="card mt-4 shadow-sm">
        <div class="card-header">
            <h5>إضافة وكالة جديدة لـ {{ $main->name ?? '' }}</h5>
        </div>

        <div class="card-body">
            <form action="{{ route('agencies.store', $main->id) }}" method="POST">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3 position-relative">
                        ...
                        <input type="text" name="user_name" id="user_name"
                            class="form-control @error('user_id') is-invalid @enderror" value="{{ old('user_name') }}"
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="lawyers" id="lawyers"
                            class="form-control @error('lawyers') is-invalid @enderror" value="{{ old('lawyers') }}"
                            required>
                        @error('lawyers')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="opponents" id="opponents"
                            class="form-control @error('opponents') is-invalid @enderror" value="{{ old('opponents') }}"
                            required>
                        @error('opponents')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="court" id="court"
                            class="form-control @error('court') is-invalid @enderror" value="{{ old('court') }}" required>
                        @error('court')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="for" id="for"
                            class="form-control @error('for') is-invalid @enderror" value="{{ old('for') }}" required>
                        @error('for')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <button type="submit" class="btn btn-success">حفظ الوكالة</button>
                <a href="{{ route('agencies.index', $main) }}" class="btn btn-secondary">إلغاء</a>
            </form>
        </div>
    </div>
@endsection
